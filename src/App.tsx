...
const generateMentorResponse = (trigger, context = {}, userMessage = '') => {
  // Added default values to avoid undefined reference errors
  const {
    hoursToday = 0,
    targetHours = 10,
    daysLeft = 0,
  ...
  const responses = {
    check_in: [
      ...
      `You need ${targetHours} hours today. Abhi sirf ${Number(
        hoursToday
      ).toFixed(1)} hue hain. Speed up.`,
    ],
    missed_target: [
    ...
    urgency_90_plus:
      "You have time. Build strong concepts. But don't waste days.",
    urgency_30_to_90:
      'Now consistency matters. No more delays. Every single day counts.',
    urgency_less_30:
      'Final phase. Every hour counts. Drop everything else and focus.',
  ...
  if (responses[trigger]) {
    ...
    // Increase harshness based on escalation
  ...
  return responses.general_chat[
    Math.floor(Math.random() * responses.general_chat.length)
  ];
};

export default function CASathiApp() {
  // --- STATE MANAGEMENT (Now with Memory!) ---
  ...
  // Stats
  const [hoursStudiedToday, setHoursStudiedToday] = useLocalStorage(
    'ca-hoursToday',
    0
  );
  const [streak, setStreak] = useLocalStorage('ca-streak', 0);
  const [escalationLevel, setEscalationLevel] = useLocalStorage(
    'ca-escalation',
    0
  );

  // Computed
  const rawDaysLeft = examDate
    ? Math.ceil((new Date(examDate) - new Date()) / (1000 * 60 * 60 * 24))
    : 0;
  const daysLeft = Math.max(1, rawDaysLeft);
  const reqDailyHours =
    hoursStudiedToday < targetHours
  ...
  const handleAddTask = (e) => {
    ...
    setTasks([...tasks, taskToAdd]); // Adds new task to the list
    setShowAddTaskModal(false); // Closes the popup
    setNewTask({
      subject: '',
      topic: '',
      duration: 2,
  ...
  const handleSessionComplete = () => {
    if (timerMode === 'pomodoro') {
      ...
      // Play harsh alarm sound in real app
    } else {
  ...
  const logSessionResult = (status) => {
    ...
    if (status === 'completed') {
      ...
      addMentorMessage(
        generateMentorResponse('good_session', {
      ...
      setTimeLeft(10 * 60); // 10 min break
      setIsActive(true);
    } else if (status === 'partial') {
      setHoursStudiedToday((prev) => prev + 25 / 60);
      addMentorMessage(
        'Partial completion is just a polite word for procrastination. Why did you stop?'
      );
      setEscalationLevel((prev) => prev + 1);
    } else {
      addMentorMessage(
        generateMentorResponse('missed_target', {
          hoursToday: hoursStudiedToday,
          targetHours,
          daysLeft,
          streak,
  ...
  // Paste your Gemini API key inside these quotes:
  ...
  const handleChatSubmit = async (e) => {
    ...
    // 1. Show a loading message so the app feels responsive
    ...
    try {
      ...
      // Check if Google sent an error back instead of an answer
      if (data.error) {
        throw new Error(`Google API Refused: ${data.error.message}`);
      }

      const aiReply = data.candidates[0].content.parts[0].text;
      // 4. Update the chat history to replace the "..." with the real answer
      setChatHistory((prev) => {
      ...
      setChatHistory((prev) => {
        ...
        newHistory[newHistory.length - 1] = {
          sender: 'mentor',
          text: `SYSTEM ERROR: ${error.message}. (If it says "Failed to fetch", turn off your ad-blocker or brave shields!)`,
          time: new Date().toLocaleTimeString([], {
            hour: '2-digit',
            minute: '2-digit',
          }),
    ...
    <div className="space-y-6">
      ...
      <div className="bg-red-950/40 border border-red-900/50 rounded-xl p-6 relative overflow-hidden">
        ...
        <div className="flex justify-between items-center relative z-10">
          <div>
            <h2 className="text-red-500 font-bold tracking-wider text-sm mb-1 uppercase">
              Mission CA Final
            </h2>
            ...
            <p className="mt-2 text-red-200/80 text-sm font-medium border-l-2 border-red-500 pl-3">
              "{generateMentorResponse('urgency', { daysLeft })}"
            </p>
          </div>
          <div className="text-right">
            ...
            <input
              ...
              className="bg-black/50 border border-gray-700 rounded px-3 py-1 text-white font-mono text-sm focus:outline-none focus:border-red-500"
            />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gray-800/50 border border-gray-700/50 p-5 rounded-xl flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-bold">
              {hoursStudiedToday.toFixed(1)} / {targetHours}{' '}
            ...
            <div className="w-full bg-gray-700 h-1.5 mt-2 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${(hoursStudiedToday / targetHours) * 100}%` }}
              ></div>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div>
            ...
            <div className="text-2xl font-bold">
              {reqDailyHours}{' '}
              <span className="text-sm font-normal text-gray-500">hrs/day</span>
            </div>
            <div
      ...
      <div className="bg-gray-800/30 border border-gray-700/50 rounded-xl p-6">
        <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
          <Calendar size={18} className="text-blue-400" /> Today's Battle Plan
        </h3>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-400 mt-0.5">
                    {task.timeOfDay} Block • {task.duration} Hours
                  </div>
                </div>
              </div>
              <div className="text-right">
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] bg-gray-900/50 border border-gray-700/50 rounded-xl overflow-hidden">
      <div className="p-4 bg-gray-800 border-b border-gray-700 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-red-900/50 rounded-full flex items-center justify-center border border-red-500/30 text-red-400">
            <BrainCircuit size={20} />
          </div>
          <div>
            <h3 className="font-bold text-gray-100">AI Mentor (Strict Mode)</h3>
        ...
        <div className="text-xs text-gray-500 font-mono">
          Escalation Lvl: {escalationLevel}
        </div>
      </div>
            ...
            >
              ...
              <div
                className={`text-[10px] text-right opacity-60 ${
                  msg.sender === 'mentor' && msg.text.includes('acceptable')
                    ? 'text-red-300'
                    : ''
                }`}
              >
      ...
      >
        <input
          ...
          placeholder="Update your mentor or ask a doubt..."
  ...
  const renderTimer = () => (
    <div className="h-full flex flex-col items-center justify-center">
      <div className="text-center w-full max-w-md">
        ...
        <div
          className={`text-8xl font-black font-mono tracking-tighter mb-12 ${
        ...
        <div className="flex justify-center gap-6 mb-12">
          ...
          <button
            onClick={() => {
              setIsActive(false);
              setTimeLeft(timerMode === 'pomodoro' ? 50 * 60 : 10 * 60);
            }}
            className="w-16 h-16 rounded-full flex items-center justify-center bg-gray-800 border border-gray-700 text-gray-400 hover:text-white transition-all"
          >
            <RotateCcw size={24} />
          </button>
        </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-8 max-w-sm w-full text-center">
            ...
            <p className="text-gray-400 text-sm mb-6">
              Mentor is asking: How much did you actually study?
            </p>

            <div className="space-y-3">
              <button
                onClick={() => logSessionResult('completed')}
                className="w-full py-3 bg-green-900/40 text-green-400 border border-green-800 hover:bg-green-800/60 rounded-lg font-bold flex items-center justify-center gap-2 transition-colors"
              >
                <CheckCircle size={18} /> Fully Completed (100% Focus)
              </button>
              <button
                onClick={() => logSessionResult('partial')}
                className="w-full py-3 bg-yellow-900/40 text-yellow-400 border border-yellow-800 hover:bg-yellow-800/60 rounded-lg font-bold flex items-center justify-center gap-2 transition-colors"
              >
                <AlertTriangle size={18} /> Partially Completed (Distracted)
              </button>
              <button
                onClick={() => logSessionResult('failed')}
                className="w-full py-3 bg-red-900/40 text-red-400 border border-red-800 hover:bg-red-800/60 rounded-lg font-bold flex items-center justify-center gap-2 transition-colors"
              >
                <XCircle size={18} /> Wasted Session (0 Output)
              </button>
            </div>
          </div>
        </div>
    ...
    <div className="space-y-6 relative">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors shadow-[0_0_10px_rgba(37,99,235,0.4)]"
        >
          + Add New Task
        </button>
      </div>
      ...
      <div className="bg-gray-800/30 border border-gray-700/50 rounded-xl p-6">
        <p className="text-gray-400 text-sm mb-6 border-l-4 border-blue-500 pl-3">
          System automatically assigns Hard subjects (FR/AFM) to Morning, Medium
          to Afternoon, and Revisions to Night to optimize energy levels.
        </p>
            ...
            <tbody className="divide-y divide-gray-800">
              {tasks.map((task) => (
                <tr key={task.id} className="text-gray-200">
                  <td className="py-4 font-medium">
                    {task.subject}:{' '}
                    <span className="text-gray-400">{task.topic}</span>
                  </td>
                  ...
                  <td className="py-4">
                    <button
                      ...
                      className="text-red-500/70 hover:text-red-400 transition-colors text-xs font-medium"
                    >
                      Delete
                    </button>
                  </td>
              ...
              {tasks.length === 0 && (
                <tr>
                  ...
                  >
                    No tasks assigned for today. Add a task to begin.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 max-w-md w-full shadow-2xl">
            <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
              <BookOpen size={20} className="text-blue-400" /> Add Study Target
            </h3>
            <form onSubmit={handleAddTask} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-gray-800 border border-gray-700 rounded p-2 text-sm text-white outline-none focus:border-blue-500 transition-colors"
                    required
                  />
                </div>
                <div>
              ...
              <div className="flex gap-3 mt-8 pt-4 border-t border-gray-800">
                <button
                  ...
                  className="flex-1 py-2.5 bg-gray-800 text-gray-300 rounded-lg hover:bg-gray-700 transition-colors text-sm font-medium"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-500 transition-colors text-sm font-bold"
                >
                  Save Target
                </button>
              </div>

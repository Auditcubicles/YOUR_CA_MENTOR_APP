...
export default function CASathiApp() {
  // --- PERFECT APPLE GLASSMORPHISM THEME ---
  const [isLightMode, setIsLightMode] = useLocalStorage('ca-theme-light', false);

  const theme = {
    bg: isLightMode ? 'bg-[#f4f6f9]' : 'bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-slate-900 via-[#0a0f1c] to-black',
    ...
    cardSolid: isLightMode ? 'bg-white border-slate-200 shadow-md' : 'bg-[#0f172a]/90 border-white/10 shadow-2xl backdrop-blur-3xl',
    ...
    hoverTab: isLightMode ? 'hover:bg-slate-100 text-slate-600 hover:text-blue-600' : 'hover:bg-white/10 text-slate-300 hover:text-white',
  ...
  const [chatHistory, setChatHistory] = useLocalStorage('ca-chat', [
    { sender: 'mentor', text: "Welcome to CA Sathi. I am your AI Mentor. Let's plan your schedule and maintain absolute discipline.", time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) },
  ]);
  const [chatInput, setChatInput] = React.useState('');
  
  // Timer State (Crash-proof defaults)
  ...
  const toggleFullScreen = () => {
    if (!document.fullscreenElement) {
      timerRef.current?.requestFullscreen().catch(() => {});
    } else {
      safeExitFullscreen();
    }
  };
  ...
  // NEW: Stop Session Early & Add Partial Time
  const handleStopSession = () => {
    setIsActive(false);
    const safeWorkDur = Number(workDuration) || 50;
    const safeBreakDur = Number(breakDuration) || 10;
    const totalSecondsPlanned = timerMode === 'pomodoro' ? safeWorkDur * 60 : safeBreakDur * 60;
    ...
    if (timerMode === 'pomodoro' && secondsStudied > 0) {
      ...
      addMentorMessage(`Session stopped early. Added ${(secondsStudied / 60).toFixed(1)} mins to your daily progress.`);
    }
  ...
  const logSessionResult = (status) => {
    ...
    if (status === 'completed') {
      addMentorMessage("Good focus. Session logged perfectly. Take a short break.");
      setTimerMode('shortBreak');
      setTimeLeft(safeBreakDur * 60);
      setIsActive(true);
    } else if (status === 'partial') {
      addMentorMessage('Partial session logged. Try to avoid distractions next time.');
    } else {
  ...
  const handleChatSubmit = async (e) => {
    ...
    const prompt = `You are 'Sathi,' an elite CA Mentor. Tone: Hinglish. Professional, helpful, strict but motivating.
  ...
  const renderAnalogClock = () => {
    ...
    return (
      <div className="relative flex flex-col items-center justify-center mb-10 w-full">
        {/* SVG based clock ensures it never crashes on resizing */}
  ...
  const renderTimer = () => (
    <div ref={timerRef} className={`h-full flex flex-col items-center justify-center relative transition-all duration-300 animate-in fade-in ${isFullScreen ? (isLightMode ? 'bg-[#f4f6f9]' : 'bg-[#0a0f1c]') : ''}`}>
      ...
      <div className="text-center w-full max-w-xl px-4">
        <h2 className={`text-xs font-bold mb-8 uppercase tracking-[0.3em] ${theme.textMuted}`}>
        ...
        {timerDisplayType === 'digital' ? (
          <div className={`text-6xl md:text-[8rem] font-bold font-mono tracking-tighter leading-none mb-14 drop-shadow-xl transition-colors duration-500 ${timerMode === 'shortBreak' ? 'text-emerald-500 drop-shadow-[0_0_30px_rgba(16,185,129,0.3)]' : isActive ? (isLightMode ? 'text-slate-900' : 'text-white drop-shadow-[0_0_40px_rgba(255,255,255,0.15)]') : theme.textMuted}`}>
            {formatTime(timeLeft)}
          </div>
        ) : renderAnalogClock()}

...
export default function CASathiApp() {
  ...
  const theme = {
    bg: isLightMode ? 'bg-[#FAFAFA]' : 'bg-[#09090B]', // Ultra clean zinc tones
    ...
    input: isLightMode ? 'bg-slate-50 border-slate-200 text-slate-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500' : 'bg-[#09090B] border-zinc-700 text-zinc-100 focus:border-blue-500 focus:ring-1 focus:ring-blue-500',
  ...
  const handleSessionComplete = () => {
    if (timerMode === 'pomodoro') {
      ...
      addMentorMessage('Break over. Back to your books.');
    }
  };
  ...
  const handleStopSession = () => {
    ...
    if (timerMode === 'pomodoro' && secondsStudied > 0) {
      ...
      addMentorMessage(`Session stopped. Added ${(secondsStudied / 60).toFixed(1)} mins to your progress.`);
    }
  ...
  const logSessionResult = (status) => {
    ...
    if (status === 'completed') {
      addMentorMessage("Session logged perfectly. Good focus.");
      setTimerMode('shortBreak');
      setTimeLeft(breakDuration * 60);
      setIsActive(true);
    } else if (status === 'partial') {
      addMentorMessage('Partial session logged. Try to maintain deeper focus next time.');
    } else {
      addMentorMessage('Session wasted. Stay disciplined.');
    }
  };
  ...
  const handleChatSubmit = async (e) => {
    ...
    const prompt = `You are 'Sathi,' a highly professional CA Mentor. Tone: Professional Hinglish. Be direct, helpful, and motivating.
    ...
    try {
      ...
      setChatHistory((prev) => {
        const newHistory = [...prev];
        newHistory[newHistory.length - 1] = { sender: 'mentor', text: `Connection error. Please try again.`, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) };
        return newHistory;
      });
    }
  };
    ...
    return (
      <div className="relative flex flex-col items-center justify-center mb-12">
        <svg width="280" height="280" viewBox="0 0 100 100" className="drop-shadow-lg">
          ...
          {[...Array(12)].map((_, i) => (
            <line key={i} x1="50" y1="6" x2="50" y2={i % 3 === 0 ? "12" : "9"} stroke={isLightMode ? '#cbd5e1' : '#3f3f46'} strokeWidth={i % 3 === 0 ? "2" : "1"} transform={`rotate(${i * 30} 50 50)`} />
          ))}
          <line x1="50" y1="50" x2="50" y2="22" stroke={isLightMode ? '#334155' : '#e4e4e7'} strokeWidth="2.5" strokeLinecap="round" transform={`rotate(${minDegree} 50 50)`} />
          <line x1="50" y1="50" x2="50" y2="15" stroke="#ef4444" strokeWidth="1" strokeLinecap="round" transform={`rotate(${secDegree} 50 50)`} />
          <circle cx="50" cy="50" r="3" fill="#3b82f6" />
        </svg>
        <div className={`absolute top-1/2 mt-12 px-3 py-1 rounded bg-black/5 border border-white/10 text-sm font-mono font-bold ${theme.text}`}>
          {formatTime(timeLeft)}
        </div>
      </div>
    );
    ...
    <div ref={timerRef} className={`h-full flex flex-col items-center justify-center relative transition-colors duration-300 animate-in fade-in ${isFullScreen ? (isLightMode ? 'bg-[#FAFAFA]' : 'bg-[#09090B]') : ''}`}>
      ...
      <div className="text-center w-full max-w-lg">
        <div className={`flex justify-center items-center gap-2 mb-10 p-1 rounded-lg border w-fit mx-auto ${isLightMode ? 'bg-slate-100 border-slate-200' : 'bg-[#18181B] border-zinc-800'}`}>
          <button onClick={() => setTimerDisplayType('digital')} className={`flex items-center gap-2 px-4 py-2 rounded-md text-xs font-semibold uppercase tracking-wider transition-all ${timerDisplayType === 'digital' ? 'bg-white text-slate-900 shadow-sm dark:bg-zinc-700 dark:text-white' : theme.textMuted}`}>
            <TimerIcon size={16} /> Digital
          </button>
          <button onClick={() => setTimerDisplayType('analog')} className={`flex items-center gap-2 px-4 py-2 rounded-md text-xs font-semibold uppercase tracking-wider transition-all ${timerDisplayType === 'analog' ? 'bg-white text-slate-900 shadow-sm dark:bg-zinc-700 dark:text-white' : theme.textMuted}`}>
        ...
        {!isActive && !isFullScreen && (
          <div className="flex justify-center gap-6 mb-10">
            <div className="flex flex-col items-center">
              ...
              <input type="number" value={workDuration} onChange={(e) => {setWorkDuration(e.target.value); if(timerMode==='pomodoro') setTimeLeft(Number(e.target.value)*60);}} className={`w-20 text-center rounded-lg p-2 font-mono text-lg font-semibold ${theme.input}`} />
            </div>
            <div className="flex flex-col items-center">
              <label className={`text-xs mb-2 font-medium uppercase tracking-wider ${theme.textMuted}`}>Break (Min)</label>
              <input type="number" value={breakDuration} onChange={(e) => {setBreakDuration(e.target.value); if(timerMode==='shortBreak') setTimeLeft(Number(e.target.value)*60);}} className={`w-20 text-center rounded-lg p-2 font-mono text-lg font-semibold ${theme.input}`} />
            </div>
          </div>
        )}

        {timerDisplayType === 'digital' ? (
          <div className={`text-6xl md:text-8xl font-semibold font-mono tracking-tight mb-12 transition-colors ${timerMode === 'shortBreak' ? 'text-emerald-500' : isActive ? theme.text : theme.textMuted}`}>
            {formatTime(timeLeft)}
          </div>
        ) : renderAnalogClock()}
        
        <div className="flex justify-center items-center gap-6 mb-12">
          <button onClick={() => { setIsActive(false); setTimeLeft(timerMode === 'pomodoro' ? workDuration * 60 : breakDuration * 60); }} className={`w-12 h-12 rounded-full flex items-center justify-center border transition-colors ${isLightMode ? 'border-slate-300 text-slate-600 hover:bg-slate-100' : 'border-zinc-700 text-zinc-400 hover:bg-zinc-800'}`} title="Reset">
            <RotateCcw size={18} />
          </button>

          <button onClick={() => setIsActive(!isActive)} className={`w-20 h-20 rounded-full flex items-center justify-center transition-all ${isActive ? 'bg-amber-500/10 text-amber-500 border border-amber-500/50 hover:bg-amber-500/20' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-lg shadow-blue-500/20'}`}>
            {isActive ? <Pause size={28} /> : <Play size={28} className="ml-1" />}
          </button>

          <button onClick={handleStopSession} className={`w-12 h-12 rounded-full flex items-center justify-center border transition-colors ${isLightMode ? 'border-red-200 text-red-500 hover:bg-red-50' : 'border-red-900 text-red-500 hover:bg-red-500/10'}`} title="Stop & Log Time">
            <StopCircle size={20} />
          </button>
        </div>
      ...
      {showSessionLog && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in">
          <div className={`${theme.cardSolid} rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl`}>
            ...
            <div className="space-y-3">
              <button onClick={() => logSessionResult('completed')} className="w-full py-3 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/20 rounded-xl font-semibold flex items-center justify-center gap-2 transition-colors"><CheckCircle size={18} /> 100% Focused</button>
              <button onClick={() => logSessionResult('partial')} className="w-full py-3 bg-amber-500/10 text-amber-600 dark:text-amber-400 hover:bg-amber-500/20 rounded-xl font-semibold flex items-center justify-center gap-2 transition-colors"><AlertTriangle size={18} /> Partially Distracted</button>
              <button onClick={() => logSessionResult('failed')} className="w-full py-3 bg-red-500/10 text-red-600 dark:text-red-400 hover:bg-red-500/20 rounded-xl font-semibold flex items-center justify-center gap-2 transition-colors"><XCircle size={18} /> Wasted (0 Output)</button>
            </div>
          </div>
        </div>
      )}
          ...
          <table className="w-full text-left text-sm">
            <thead className={`font-medium ${isLightMode ? 'bg-slate-50 text-slate-500 border-b border-slate-200' : 'bg-[#18181B] text-zinc-400 border-b border-zinc-800'}`}>
              <tr>
                ...
                <th className="p-4">Target Hrs</th>
      ...
      <aside className={`w-64 border-r flex flex-col hidden md:flex transition-colors duration-300 ${theme.sidebar} z-10`}>
        ...
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto scrollbar-hide">
          {[
            ...
            { id: 'mentor', name: 'Expert Mentor', icon: <BrainCircuit size={18} /> },
          ].map((item) => (
            <button
              key={item.id}
